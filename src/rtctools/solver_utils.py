...
def preload_custom_solver_library(
    library_path: Union[str, Path], solver_name: Optional[str] = None
) -> bool:
    """
    ...
    Supported solvers include: HiGHS, IPOPT, BONMIN, CBC, CPLEX, Gurobi, etc.
    ...
    try:
        ...
        if sys.platform.startswith("win"):
            ...
            lib = ctypes.CDLL(str(library_path), winmode=0)
        else:
        ...
        raise RuntimeError(
            ...
            f"Common causes:\n"
            f"  - Missing dependencies (check with ldd on Linux, otool on macOS, Dependencies.exe on Windows)\n"
...
def _preload_solvers_from_environment() -> None:
    ...
    for entry in env_var.split(","):
        ...
        if ":" not in entry:
            logger.warning(
                f"Invalid solver preload entry (expected 'name:path'): {entry}"
            )
            continue
...
def get_solver_library_info(solver_name: str = "highs") -> Dict[str, Union[str, Path, int]]:
    """
    ...
    **WARNING**: This function imports CasADi, which prevents any subsequent preloading.
    ...
    if env_var and "casadi" not in sys.modules:
        ...
        raise RuntimeError(
            ...
            f"To fix this:\n"
            f"  1. Complete preloading first by importing rtctools or calling preload_custom_solver_library()\n"

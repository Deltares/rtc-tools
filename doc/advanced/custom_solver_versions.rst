...
.. code-block:: dockerfile
   ...
   ``FileNotFoundError: Solver library not found`` means the path is wrong or the
   file is unreadable. Verify the path, make sure the file exists, and confirm it
   has read permissions.

   ``RuntimeError: Failed to preload`` usually indicates missing dependencies or
   an ABI mismatch. Check for missing libraries with ``ldd`` (Linux) or
   ``otool -L`` (macOS), use the `Dependencies <https://github.com/lucasg/Dependencies>`_
   tool on Windows, and confirm the build matches CasADi's compiler, bitness, and
   C++ runtime.

   The "CasADi already imported" warning appears when CasADi loads before
   preloading runs (for example, after importing ``rtctools.optimization``
   directly). Import ``rtctools`` or call ``preload_custom_solver_library()``
   before any CasADi-dependent modules. Use ``check_preload_import_order()`` if
   you want the process to fail fast when the order is wrong.

   If the solver still reports the bundled version, check logs for preload errors,
   confirm the import order, and inspect ``get_solver_library_info()`` for the
   active paths.

   Compatibility Notes
   ^^^^^^^^^^^^^^^^^^^

   * **HiGHS**: Only solver validated with this guide; its C API has been stable
     since v1.7.0.
   * Other CasADi solvers may work with the same approach, but they have not been
     officially tested. Validate thoroughly before relying on them in production.
   * Requires Python 3.7+ (``ctypes`` standard library) and CasADi 3.6.0 or newer
     for the preload hooks used here.

   Solver Coverage
   ^^^^^^^^^^^^^^^

   We have confirmed this workflow with HiGHS. If you preload other solvers
   (IPOPT, BONMIN, CBC, etc.), treat them as experimental and perform end-to-end
   validation. Commercial solvers typically ship their own integration tooling and
   may not benefit from this preload pathway.

   Testing Your Custom Solver Setup
   ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
   ...
   **Integration Testing:**
   ...
   2. **Verify preloading** - Use ``get_solver_library_info()`` to confirm the custom library loaded.
   3. **Document versions** - Keep records of which solver builds work with your application.
   4. **Use environment variables in production** - They are easier to manage in deployment pipelines.
   5. **Version pin** - Specify exact solver versions in your documentation or release notes.
   6. **Container builds** - Bundle custom solvers with Docker images for reproducibility.
   7. **CI/CD integration** - Exercise the custom solver in continuous integration to catch regressions early.
   ...
   .. code-block:: python
      ...
      # 1. Preload custom solver (must be first!)
      preload_custom_solver_library('/opt/highs-1.12/lib/libhighs.so', 'HiGHS')

      # 2. Import optimization modules
      from rtctools.optimization.collocated_integrated_optimization_problem import (
         CollocatedIntegratedOptimizationProblem
      )

      # 3. Define and run your optimization
      class MyOptimization(CollocatedIntegratedOptimizationProblem):
      ...
      problem = MyOptimization()
      problem.optimize()

   API Reference
   ^^^^^^^^^^^^^

   For complete API details see ``rtctools.solver_utils`` docstrings:

   * ``preload_custom_solver_library(library_path, solver_name=None)``
   * ``check_preload_import_order()``
   * ``get_solver_library_info(solver_name='highs')``
   * ``RTCTOOLS_PRELOAD_SOLVER_LIBS`` environment variable format
   from rtctools.solver_utils import get_solver_library_info
...
This must run in a fresh Python process since CasADi cannot be reloaded once imported.
